...
def train(config, pretrained=None):
    ...
    prefix = config['model']
    num = config['num']
    max_epochs = config['max_epochs']  # 添加這一行來設置最大訓練週期
    ...
    epoch_r = min(int(300000 / len_dl), max_epochs)  # 修改這一行來使用最大訓練週期參數
    for epoch in range(epoch_r):
        ...
        if (epoch + 1) % 50 == 0:
            torch.save(net.state_dict(),
                       f'{prefix}_epoch_{epoch + 1}_{num}.pth')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--max_epochs', type=int, default=100, help='Maximum number of training epochs')  # 添加這一行來設置最大訓練週期參數
