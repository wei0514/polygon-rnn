...
for file in files:
    ...
    for obj in objects:
        if obj['label'] in selected_classes:
            min_c = np.min(np.array(obj['points']), axis=0)
            max_c = np.max(np.array(obj['points']), axis=0)
            ...
            dic = {}
            dic['points'] = []
            for p_count, points in enumerate(obj['points']):
                ...
                dic['points'].append([index_a, index_b])
